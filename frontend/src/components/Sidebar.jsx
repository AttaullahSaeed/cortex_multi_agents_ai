...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  // ✅ Fixed: always call when userData changes / mounts
  useEffect(() => {
    if (!userData) return;
  ...
  if (collapse) {
    return (
      <div className="hidden lg:flex flex-col items-center w-[56px] h-screen bg-[#0d0f14] border-r border-white/[0.06] py-4 gap-1 shrink-0">
        ...
        <div className="flex-1 pt-5 overflow-y-auto px-2.5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {conversations?.map((conv, i) => {
            ...
            return (
              <div
                ...
                className={`flex items-center justify-center cursor-pointer mb-0.5 p-2 rounded-[10px] border transition-colors ${
                  isActive
        ...
        <div className="relative shrink-0">
          {userData?.avatar && !imageError ? (
            ...
            <div className="w-9 h-9 rounded-[10px] bg-white/[0.06] flex justify-center items-center">
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]">
          ...
          <button
            ...
            className="flex lg:hidden items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors bg-transparent border-none cursor-pointer"
          >
            <X size={16} />
          </button>
          ...
          <button
            onClick={handleCreateConversation}
            className="hidden md:flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors bg-transparent border-none cursor-pointer"
          >
            <PenSquare size={14} />
          </button>
        </div>
          ...
          {conversations?.map((conv, i) => {
            ...
            return (
              <div
                key={conv?._id || i}
                onClick={() => handleSelectConversation(conv)}
                className={`flex items-center gap-2.5 cursor-pointer mb-0.5 px-3 py-2.5 rounded-[10px] border transition-colors ${
                  isActive
                    ? "bg-indigo-500/10 border-indigo-500/[0.18]"
                    : "bg-transparent border-transparent hover:bg-white/[0.04]"
                }`}
        ...
        <div className="px-3.5 py-3.5">
          {userData ? (
            <div className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 hover:bg-white/[0.05] transition-colors">
              <div className="relative shrink-0">
                {userData?.avatar && !imageError ? (
                  <img
                    src={userData.avatar}
              ...
              <div className="flex gap-1">
                <button className="flex items-center justify-center w-7 h-7 rounded-[7px] border-none bg-transparent text-yellow-600 cursor-pointer hover:bg-white/[0.08] transition-all">
                  <Coins size={16} />
                </button>
                <button
                  onClick={() => {
                  ...
                  className="flex items-center justify-center w-7 h-7 rounded-[7px] border-none bg-transparent text-slate-600 cursor-pointer hover:bg-white/[0.08] hover:text-slate-400 transition-all"
                >
                  <LogOut size={16} />
                </button>
              </div>
            ...
            <button className="w-full text-sm text-slate-400">Login</button>
          )}
        </div>
      </div>
    </div>
